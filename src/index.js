...
const checkEndpoint = (url, endpoints) => {
  const host = parseHostFromUrl(url)
  return endpoints.findIndex(i => i === host) > -1
}
...
const parseError = err => (err && err.response && err.response.data ? err.response.data : err)
...
class Superlogin extends EventEmitter2 {
  ...
  checkRefresh() {
    ...
    const threshold =
      this._config.refreshThreshold !== undefined ? this._config.refreshThreshold : 0.5
  ...
  register(registration) {
    return this._http
      .post(`${this._config.baseUrl}/register`, registration, { skipRefresh: true })
      .then(res => {
  ...
  tokenSocialAuth(provider, accessToken) {
    ...
    return this._http
      .post(`${this._config.baseUrl}/${provider}/token`, { access_token: accessToken })
      .then(res => {
        if (res.data.user_id && res.data.token) {
          res.data.serverTimeDiff = res.data.issued - Date.now()
          this.setSession(res.data)
  ...
  forgotPassword(email) {
    return this._http
      .post(`${this._config.baseUrl}/forgot-password`, { email }, { skipRefresh: true })
      .then(res => res.data)
      .catch(err => {
        throw parseError(err)
      })
  ...
  resetPassword(form) {
    return this._http
      .post(`${this._config.baseUrl}/password-reset`, form, { skipRefresh: true })
      .then(res => {
        if (res.data.user_id && res.data.token) {
          this.setSession(res.data)
          this._onLogin(res.data)
  ...
  validateUsername(username) {
    return this._http
      .get(`${this._config.baseUrl}/validate-username/${encodeURIComponent(username)}`)
      .then(() => true)
      .catch(err => {
        throw parseError(err)
      })
  ...
  validateEmail(email) {
    return this._http
      .get(`${this._config.baseUrl}/validate-email/${encodeURIComponent(email)}`)
      .then(() => true)
      .catch(err => {
        throw parseError(err)
      })
